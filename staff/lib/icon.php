...
/**
 * スタッフ画面用のインラインSVGアイコンを出力する。
 *
 * フォントの読み込み状態に左右されないため、アイコン名が文字として
 * 表示されることがない。既存CSSとの互換性のため material-icons も残す。
 */
function staff_icon(
    string $name,
    string $class = '',
    ...
    $name = trim($name) ?: 'circle';
    $size = max(1, $size);

    $aliases = [
        'account_circle' => 'user', 'manage_accounts' => 'users', 'group' => 'users',
        'groups' => 'users', 'person_add' => 'user-plus', 'person_off' => 'user-x',
        'badge' => 'user', 'admin_panel_settings' => 'shield', 'verified_user' => 'shield',
        'approval' => 'check-circle', 'check' => 'check', 'check_circle' => 'check-circle',
        'task_alt' => 'check-circle', 'done_all' => 'check-circle',
        'home' => 'home', 'dashboard' => 'grid', 'space_dashboard' => 'grid',
        'dashboard_customize' => 'grid', 'apps' => 'grid', 'view_list' => 'list',
        'view_stream' => 'list', 'menu_open' => 'menu', 'category' => 'grid',
        'settings' => 'settings', 'settings_suggest' => 'settings', 'tune' => 'sliders',
        'palette' => 'palette', 'search' => 'search', 'filter_alt' => 'filter',
        'sort' => 'sort', 'close' => 'close', 'logout' => 'logout',
        ...
        'open_in_new' => 'external-link', 'add' => 'plus', 'add_box' => 'plus-square',
        'add_link' => 'link', 'link' => 'link', 'sync' => 'refresh',
        'restart_alt' => 'refresh', 'swap_horiz' => 'swap',
        'notifications' => 'bell', 'notifications_active' => 'bell',
        'notification_important' => 'bell', 'campaign' => 'megaphone',
        'mail' => 'mail', 'reply' => 'reply', 'send' => 'send', 'inbox' => 'inbox',
        'forum' => 'message', 'support_agent' => 'headphones', 'edit_note' => 'edit',
        'description' => 'file', 'newspaper' => 'file', 'receipt_long' => 'receipt',
        'request_quote' => 'receipt', 'assignment' => 'clipboard',
        'language' => 'globe', 'domain' => 'building', 'location_on' => 'pin',
        'web_asset' => 'monitor', 'computer' => 'monitor', 'monitoring' => 'activity',
        'monitor_heart' => 'activity', 'dns' => 'server', 'storage' => 'database',
        'database' => 'database', 'memory' => 'cpu', 'hub' => 'network',
        'account_tree' => 'network', 'terminal' => 'terminal', 'code' => 'code',
        'rocket_launch' => 'rocket', 'play_circle' => 'play',
        'precision_manufacturing' => 'cpu', 'sports_esports' => 'gamepad',
        'inventory_2' => 'box', 'warehouse' => 'building', 'shopping_bag' => 'bag',
        'sell' => 'tag', 'payments' => 'credit-card', 'qr_code_2' => 'qr',
        'qr_code_scanner' => 'qr', 'lock' => 'lock', 'key' => 'key',
        'info' => 'info', 'error' => 'alert', 'error_outline' => 'alert',
        'warning' => 'alert', 'history' => 'history', 'add_photo_alternate' => 'image',
        'delete' => 'trash', 'save' => 'save', 'workspaces' => 'grid',
    ];
    ...
    $body = $paths[$key] ?? $paths['circle'];
    $classes = trim('material-icons staff-svg-icon ' . $class);
    $attributeParts = [
        ...
        'aria-hidden="true"',
        'focusable="false"',
        'data-icon="' . htmlspecialchars($name, ENT_QUOTES, 'UTF-8') . '"',
    ];

    foreach ($attributes as $attribute => $value) {
        $attribute = trim((string) $attribute);
        if ($attribute === '' || in_array($attribute, ['class', 'width', 'height', 'viewBox', 'style'], true)
            || !preg_match('/^[a-zA-Z_:][a-zA-Z0-9_:.-]*$/', $attribute)) {
            continue;
        }
        if (is_bool($value)) {
            if ($value) {
                $attributeParts[] = htmlspecialchars($attribute, ENT_QUOTES, 'UTF-8');
            }
            continue;
        }
        if ($value !== null) {
    ...
    return '<svg ' . implode(' ', $attributeParts) . '>' . $body . '</svg>';
}
