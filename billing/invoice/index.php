...
$pageTitle = "請求詳細 | HC Platform";
$pageDescription = "HC Platformの請求詳細ページです。";
...
function invoice_datetime(?string $value): string
{
    if (!$value) {
        return "-";
    }
...
if (!$orderId) {
    ...
    try {
        ...
        if (!$order) {
            $errors[] = "請求情報が見つからないか、表示権限がありません。";
        }
    } catch (Throwable $e) {
        ...
        <div class="container">
            <div class="invoice-toolbar">
                ...
                <?php if ($order): ?>
                    ...
                    <a href="/billing/payment-method/?order_id=<?php echo h((string)$order["id"]); ?>" class="sub-button">支払い方法変更</a>
                <?php endif; ?>
            </div>
            ...
            <?php if ($order): ?>
                <section class="invoice-document reveal">
                    ...
                    <div class="invoice-meta-grid">
                        <section>
                            <h3>請求先</h3>
                            <p><?php echo h((string)$order["username"]); ?></p>
                    ...
                    <div class="invoice-notice">
                        <strong>これは正式な請求書ではありません。</strong>
                        <p>
                            現在は契約情報から作成した仮の請求詳細です。
                            Stripe連携後、正式な請求書番号・領収書・支払い履歴を表示します。
                        </p>
                    </div>
                </section>
            <?php endif; ?>
